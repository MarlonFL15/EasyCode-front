...
class Roullete extends React.Component {
  ...
  getColor(index) {
    const colors = ['rgba(23,87,228,0.4)', 'rgba(191,17,100,0.4)','rgba(182,138,117,0.4)',
                    'rgba(248,121,17,0.4)', 'rgba(109,253,91,0.4)']
    return colors[index]
  }
  ...
  render() {
    return (
      <div className="root">
        <div>rode a roleta e blablabla</div>
        ...
        <div class="display">
          <span id="readout">
            {this.state.list[this.state.result]?alert(this.state.list[this.state.result]):false}
            </span>
        </div>
      </div>
    );
